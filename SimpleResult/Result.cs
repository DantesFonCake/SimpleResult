...
public static class Result
{
		...
		Func<Exception, TError> exceptionMapper)
	{
		try
		{
			return new Result<TValue, TError>(func());
		}
		catch (Exception e)
		{
			return new Result<TValue, TError>(exceptionMapper(e));
		}
	}
	
	public static Result<TValue, Exception> Try<TValue>(Func<TValue> func) => Try(func, e => e);
}
